...
public class NetworkInfo : MonoBehaviour {
  ...
  public void SetAuthorityIndex(int index) { m_authorityIndex = index; }
  ...
  public bool IsHeldByRemotePlayer(RemoteAvatar avatar, RemoteAvatar.HandData hand) { return m_remoteAvatar == avatar && m_remoteHand == hand; }
  ...
  public bool CanLocalPlayerGrabCube(ulong gripInputStartFrame) {
    if (m_holdClientIndex == -1) return true;
    if (m_localHand?.gripObjectStartFrame < gripInputStartFrame) return true;
  ...
  public void AttachCubeToLocalPlayer(Avatar avatar, Avatar.HandData h) {
    ...
    h.gripObjectSupportList = m_context.GetSupportList(h.gripObject);
  ...
  void OnCollisionEnter(Collision c) {
    ...
    m_context.CollisionCallback(cubeId1, cubeId2, c);
  }
